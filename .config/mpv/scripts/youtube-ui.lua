...
opt.read_options(user_opts, "osc", function(list)
	update_options(list)
end)
...
local osc_styles = {
	...
	seekbar_fg = "{\\blur0\\bord0\\1c&HEBCB8B}",
	volumebar_bg = "{\\blur0\\bord0\\1c&HFFFFFF}",
	volumebar_fg = "{\\blur0\\bord0\\1c&HEBCB8B}",
	...
	for n = 1, #elements do
		...
		if element.type == "slider" then
			...
			if pos then
				...
				-- active bar
				ass_draw_rr_h_cw(elem_ass, pad + r - bar_r, foH - bar_r, xp, foH + bar_r, bar_r, false, 0)

				-- handle
				ass_draw_rr_h_cw(elem_ass, xp - r, foH - r, xp + r, foH + r, r, false)
				...
				if mouse_hit(element) and activatable then
					...
					if element.thumbnail and not thumbfast.disabled then
						...
						if osd_w then
							...
							local thumb_x = math.min(
								osd_w - thumbfast.width - thumb_margin_x,
							...
							elem_ass:rect_cw(
								-thumb_pad * r_w,
								-thumb_pad * r_h,
								(thumbfast.width + thumb_pad) * r_w,
								(thumbfast.height + thumb_pad) * r_h
							)
							elem_ass:draw_stop()
...
function get_playlist()
	...
	for i, v in ipairs(limlist) do
		...
		message = string.format("%s %s %s. %s\n", message, (v.current and "●" or "○"), i, title)
	end
	return message
end
...
function get_chapterlist()
	...
	for i, v in ipairs(limlist) do
		...
		message = string.format("%s %s [%s] %s\n", message, (v.current and "●" or "○"), time, title)
	end
	return message
end
...
function window_controls()
	...
	lo.geometry = { x = titlebox_left + left_pad, y = wc_geo.y, an = 1, w = titlebox_w, h = wc_geo.h }
	lo.style = string.format(
		"%s{\\clip(%f,%f,%f,%f)}",
		osc_styles.title,
		titlebox_left + left_pad,
		wc_geo.y - wc_geo.h,
		titlebox_right - right_pad,
		wc_geo.y + wc_geo.h
	)
...
function validate_user_opts()
	...
	if user_opts.windowcontrols_alignment ~= "right" and user_opts.windowcontrols_alignment ~= "left" then
		msg.warn('windowcontrols_alignment cannot be "' .. user_opts.windowcontrols_alignment .. '". Ignoring.')
...
function osc_init()
	...
	ne.eventresponder["mbtn_right_down"] = function()
		mp.commandv("seek", -60, "relative", "exact")
	end
	...
	ne.content = function()
		...
		local volicon = { icons.volume_low, icons.volume_medium, icons.volume_high, icons.volume_over }
	...
	ne.eventresponder["wheel_up_press"] = function()
		mp.commandv("seek", 5, "relative-percent", "exact")
	end
	ne.eventresponder["wheel_down_press"] = function()
		mp.commandv("seek", -5, "relative-percent", "exact")
	end
	...
	ne.content = function()
		...
		if state.rightTC_trem then
			...
			if state.tc_ms then
				return (
					mp.get_property_osd("playback-time/full")
					.. " / "
					.. minus
					.. mp.get_property_osd(property .. "/full")
					.. chapter_title
				)
			else
				return (
					mp.get_property_osd("playback-time")
					.. " / "
					.. minus
					.. mp.get_property_osd(property)
					.. chapter_title
				)
			end
		else
...
function update_margins()
	...
	if
		not state.osc_visible
		or (get_hidetimeout() >= 0)
		or (state.fullscreen and not user_opts.showfullscreen)
		or (not state.fullscreen and not user_opts.showwindowed)
	then
